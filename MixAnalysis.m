...
bg_idx=zeros(1,length(File_list_NAOMI));
for i=1:length(File_list_NAOMI)
    if i==1
...
print(Fighandle,'IdealResponses_Mix','-dsvg','-r0');
...
bg_idxC=zeros(1,length(File_list_NAOMI));
for i=1:length(File_list_NAOMI)
    if i==1
...
print(Fighandle,strcat('C:\Data\Inhibited neurons\Figures\CaImAnResponses_Mix'),'-dsvg','-r0');

%Load suite2p responses

Suite2p_traces=[];
for i=1:length(File_list_NAOMI)    
    load(strcat('D:\Pictures\processed\Inhibited response\NewArtificial\Mix\',num2str(i),'\suite2p\plane0\Fall.mat'),'F');        
    if i==1
        Suite2p_traces=F;
    else
...
print(Fighandle,strcat('C:\Data\Inhibited neurons\Figures\Suite2pResponses_Mix'),'-dsvg','-r0');
...
for i=1:length(File_list_NAOMI)
    if i==1
        bg_idxC(i)=size(PCAICAResponses{i},1)+1;
    else
        bg_idxC(i)=size(PCAICAResponses{i},1)+bg_idxC(i-1)+1;
    end
end
...
print(Fighandle,strcat('C:\Data\Inhibited neurons\Figures\PCAICAResponses_Mix'),'-dsvg','-r0');
...
CaImAn_correl=cell(2,length(File_list_NAOMI));
Suite2p_correl=cell(2,length(File_list_NAOMI));
PCAICA_correl=cell(2,length(File_list_NAOMI));
